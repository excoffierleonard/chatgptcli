...
def save_chat_history(chat_history):
    ...
    if user_messages_exist:
        first_user_message = next(message["content"] for message in chat_history if message.get("role") == "user")
        formatted_message = first_user_message.replace(" ", "_")[:64]

        timestamp = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')

        filename = f"{timestamp}-{formatted_message}.json"
...
def chat_with_gpt(settings, chat_history):
...
def main():
    try:
        ...
        chat_with_gpt(settings, chat_history)
